...
async def test_basic_graph():
    ...
    print(
        f"✓ Motor state stream works: state={motor_result['state']}, confidence={motor_result['confidence']:.2f}"
    )
    ...
    print(
        f"✓ Preference reducer works: confidence={reduced.get('interaction_confidence', 0):.2f}"
    )
