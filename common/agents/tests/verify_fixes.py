...
with patch("integrations.backboard.client.backboard_client") as mock_client:
    from integrations.backboard.thread_manager import (
        ThreadManager,
        MODEL_MAPPING,
        FALLBACK_MODEL,
    )
    ...
    async def test_fallback_logic():
        ...
        tm.client.run_inference = AsyncMock(
            side_effect=[
    ...
    async def test_parsing_logic():
        ...
        with patch(
            "integrations.backboard.thread_manager.thread_manager.run_with_model",
            new_callable=AsyncMock,
        ) as mock_run:
            mock_run.return_value = valid_json
            result = await synth._infer_profile_narrative(
                "test", VisualPreferences(), InteractionPreferences(), "idle", {}, ""
            )
