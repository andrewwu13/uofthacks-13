...
class TestSSEPublisher:
    ...
    async def test_multiple_sessions_isolated(self, mock_sse_publisher):
        ...
        await mock_sse_publisher.publish_layout_update("session_1", {"id": 1})
        await mock_sse_publisher.publish_layout_update("session_2", {"id": 2})

        assert len(mock_sse_publisher.published_messages) == 2

        session_1_updates = [
            m
            for m in mock_sse_publisher.published_messages
...
class TestHealthCheckEndpoint:
    ...
    def test_health_check_returns_200(self):
        ...
        with patch("app.db.redis_client.redis_client") as mock_redis, patch(
            "app.db.mongo_client.mongo_client"
        ) as mock_mongo:

            mock_redis.health_check = AsyncMock(
                return_value={"status": "connected", "latency_ms": 0.5}
            )
            mock_mongo.health_check = AsyncMock(
                return_value={"status": "connected", "latency_ms": 1.0}
            )
    ...
    def test_health_check_returns_status(self):
        ...
        with patch("app.db.redis_client.redis_client") as mock_redis, patch(
            ...
            mock_redis.health_check = AsyncMock(return_value={"status": "connected"})
            mock_mongo.health_check = AsyncMock(return_value={"status": "connected"})
