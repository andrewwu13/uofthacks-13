...
class TestConstraintBuilder:
    ...
    def test_build_constraints_dark_mode(self):
        ...
        output = ReducerOutput(visual=VisualTraits(color_scheme="dark", density="low"))
...
class TestComponentSelector:
    ...
    def test_select_components(self):
        ...
        result = component_selector.select(
            constraints=constraints, required_types=["hero", "product-grid"]
        )
    ...
    def test_recently_used_exclusion(self):
        ...
        first_hero = next(
            (c for c in result1.selected_components if c.component_type == "hero"), None
        )

        # Second selection with first hero as recently used
        if first_hero:
            result2 = component_selector.select(
                constraints=constraints, recently_used={first_hero.component_id}
            )
            second_heroes = [
                c for c in result2.selected_components if c.component_type == "hero"
            ]
...
class TestLayoutAssembler:
    ...
    def test_assemble_layout(self):
        ...
        layout = layout_assembler.assemble(
            session_id="test_layout_001", selection=selection, reducer_output=output
        )
    ...
    def test_layout_hash_determinism(self):
        ...
        layout1 = layout_assembler.assemble(
            session_id="test_hash", selection=selection, reducer_output=output
        )
