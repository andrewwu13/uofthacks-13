...
class TestMotorSampleTransformation:
    ...
    def test_single_sample(self):
        ...
        motor = MotorTelemetryPayload(
            session_id="test", device="mouse", t0=1000, dt=16, samples=[[100, 200]]
        )
    ...
    def test_velocity_calculation(self):
        ...
        motor = MotorTelemetryPayload(
            ...
            t0=1000,
            dt=100,  # 100ms = 0.1s
            samples=[[0, 0], [10, 0]],  # Moved 10 pixels in 0.1s = 100 px/s
        )
...
class TestTelemetryEndpoint:
    ...
    def mock_app_dependencies(self, mock_redis, mock_mongo):
        ...
        with patch("app.db.redis_client.redis_client", mock_redis), patch(
            "app.db.mongo_client.mongo_client", mock_mongo
        ), patch("app.api.events.redis_client", mock_redis), patch(
            "app.api.events.mongo_client", mock_mongo
        ):
    ...
    def test_endpoint_accepts_valid_payload(self, sample_telemetry_batch):
        ...
        with patch("app.db.redis_client.redis_client"), patch(
            "app.db.mongo_client.mongo_client"
        ), patch("app.api.events.redis_client"), patch("app.api.events.mongo_client"):
...
class TestProcessTelemetryBatch:
    ...
    def mock_all_deps(self, mock_redis, mock_mongo, mock_sse_publisher):
        ...
        patches = {
            ...
            "agent": patch(
                "app.api.events.run_layout_generation", AsyncMock(return_value=None)
            ),
    ...
    async def test_acquires_and_releases_lock(
        self, mock_all_deps, sample_telemetry_batch
    ):
    ...
    @pytest.mark.asyncio
    async def test_publishes_sse_update(
        self, mock_redis, mock_mongo, mock_sse_publisher, sample_telemetry_batch
    ):
        ...
        with patch("app.api.events.redis_client", mock_redis), patch(
            "app.api.events.mongo_client", mock_mongo
        ), patch("app.api.events.sse_publisher", mock_sse_publisher), patch(
            "app.api.events.run_layout_generation", AsyncMock(return_value=None)
        ), patch(
            "app.api.events.SEMANTIC_CACHE_AVAILABLE", False
        ):
            ...
            assert (
                mock_sse_publisher.published_messages[0]["session_id"]
                == batch.session_id
            )
