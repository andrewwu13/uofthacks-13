...
def profile_to_vector(profile: ReducerOutput) -> FeatureVector:
    ...
    # Corner radius
    vector[FeatureIndex.CORNER_ROUNDNESS] = encode_value(
    ...
    # Typography weight
    vector[FeatureIndex.TYPOGRAPHY_WEIGHT] = encode_value(
        "typography_weight", visual.typography_weight
    )
    ...
    exploration_factor = encode_value(
        "exploration_tolerance", interaction.exploration_tolerance
    )
    loudness_score = (
        vector[FeatureIndex.VIBRANCY] * 0.4
    ...
    vector[FeatureIndex.INTERACTIVITY] = encode_value(
        "engagement_depth", behavioral.engagement_depth
    )
...
def user_profile_to_vector(profile_dict: dict) -> FeatureVector:
    ...
    reducer = ReducerOutput(
        ...
        interaction=(
            InteractionTraits(**interaction) if interaction else InteractionTraits()
        ),
