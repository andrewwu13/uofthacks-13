...
KEY_SCHEMA = {
    "session:{id}:state": {
        "ttl": TTL.SESSION,
        "type": "json",
        "purpose": "Latest reducer output",
    },
    "session:{id}:constraints": {
        "ttl": TTL.SESSION,
        "type": "json",
        "purpose": "Hard/soft constraints",
    },
    "session:{id}:exploration_budget": {
        "ttl": TTL.SESSION,
        "type": "float",
        "purpose": "0.0-1.0",
    },
    "session:{id}:candidates": {
        "ttl": TTL.CANDIDATES,
        "type": "json_list",
        "purpose": "Top-K component IDs",
    },
    "session:{id}:selected": {
        "ttl": TTL.CANDIDATES,
        "type": "json_list",
        "purpose": "Final component IDs",
    },
    "session:{id}:layout_hash": {
        "ttl": TTL.LAYOUT,
        "type": "string",
        "purpose": "SHA256 of layout",
    },
    "session:{id}:layout": {
        "ttl": TTL.LAYOUT,
        "type": "json",
        "purpose": "Cached layout schema",
    },
    "session:{id}:recently_used": {
        "ttl": TTL.RECENTLY_USED,
        "type": "set",
        "purpose": "Component ID set",
    },
}
